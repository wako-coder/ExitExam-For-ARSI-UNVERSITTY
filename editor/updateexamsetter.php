...
<div style="margin: 10px;float: right;border:1px Solid #dadcdc;
 ...
 <div class="style1">
 <p>Dear User,<font color="red"> <?php echo $fullname;?></font> Before update or editing the Exam setter  profile,Frist you must expected to know the ID number of the registerd examsetter .After that we can edit/update the profile by enter the ID number of the Examsetternder below search textbox and then click on search button.</p>
 </div>
  <hr>
 <fieldset id="fieldset" ><legend>Update Exam Setter info</legend>
...
$d=mysql_query("select*from examsetter where sid='$searchvalue'");
...
overflow-x:scroll; margin-left: 20px;" >
	...
	<th>Mobile</th><th>photo</th><th>Email</th><th>Deparment</th><th>Exam editor ID</th></tr>";
while($row=mysql_fetch_assoc($recordfound))
{
$did=$row["sid"];
...
echo "<tr> <td>". $row1["uid"]."</td><td>".$row1["ufname"]."</td> <td>".$row1["umname"]."</td> <td>".$row1["ulname"]."</td> <td>".$row1["sex"]."</td> <td>".$row1["mobile"]."</td> <td><img src=".$row1["photo"]." width=40 height=50 alt='image'/></td> <td>".$row1["email"]."</td> <td>".$row["dname"]."</td>   <td>".$row["eid"]."</td></tr>";
}
else
echo"No Record Found";
